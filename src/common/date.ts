...
module DateUtils {
    export function getTimeDifferenceString(a: Date, b: Date): string {
        ...
        if (difference < 0) {
            ...
            const days = Math.ceil(difference / Constants.DAY_MS);
            return `<${days} ${days == 1 ? "day" : "days"}.`;
        } else if (difference >= Constants.HOUR_MS) {
            const hours = Math.ceil(difference / Constants.HOUR_MS);
            return `<${hours} ${hours == 1 ? "hour" : "hours"}.`;
        } else if (difference >= Constants.MINUTES_TO_MS) {
            const minutes = Math.ceil(difference / Constants.MINUTES_TO_MS);
